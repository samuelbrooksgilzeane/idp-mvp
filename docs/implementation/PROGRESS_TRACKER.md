...
| 9b | Multi-page UI (plan A) | COMPLETE | `beda63b` | Source and values are inspected side by side | Routed pages with deep links verified live on the deployed App; viewer sits beside the values so citing a field never navigates away. |
| 9c | Typed line candidates (plan B) | COMPLETE | `60ccb00` | Billed lines are typed and aggregatable | Five typed rows for the representative invoice summing to 881.11; the reconciliation delta of -37.31 now falls out of a plain GROUP BY. |
| 9d | Batch processing (plan C) | COMPLETE | `45f001a` | A set of documents runs in one parallel batch | Five documents under one job run at concurrency 16 in 150s wall clock against roughly 300-375s sequential, all succeeding; ineligible documents are reported without stopping the batch. |
| 9e | Summary and XLSX export (plan D) | COMPLETE | `dbc9f70` | Latest invoices can be filtered by case and exported with line detail | `make check` passes with 134 backend and 32 frontend tests; deployed to Databricks dev through the two-pass sequence with governed bootstrap run `735102880444788`. The deployed App serves `/api/results/invoices` and a real two-sheet workbook from the governed `invoice_summary` view: the representative invoice `d0ed9896` reports 5 lines, 881.11 against a stated 888.55 and a `-37.31` delta at `REVIEW_REQUIRED`. Case filtering was demonstrated on two invoices uploaded under `CASE-ALPHA` and `CASE-BETA` (batch parse `227813517718902`, batch extract `907357470375810`), scoping the documents list, the results list and both workbook sheets. A defect was found and fixed live: the reported delta omitted the `sum(line tax)` term the registered reconciliation rule includes, so a passing invoice was exported as `-300.00` beside `VALIDATED_PASS`; the report now uses the rule's signed terms and a test pins them together. |
